...
class PCA:
    ...
    def fit_data(self):
        ...
        self.fit = np.hstack((eig_pairs[0][1].reshape(self.dimensions, 1), eig_pairs[1][1].reshape(self.dimensions, 1)))

    def transform(self):
        """ Transforms data using fit (self.fit). Plots result. """
        y = self.fit.T @ (self.x.T)
        if self.choice == 0 :
            plt.scatter(y[1, :], y[0, :], c=np.arange(self.col), cmap='gist_rainbow', s=7)
        elif self.choice == 1:
            plt.scatter(y[0, :], y[1, :], c=self.labels, cmap='jet', s=7, marker=".")
        plt.show()
